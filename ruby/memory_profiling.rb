...
open(file_path, 'w') { |f|
  f.puts "date, RSS(Mbyte), Object count on heap"
...
Thread.new do
  ...
  while true
    rss_in_mbyte = `ps -eo pid,rss | grep #{pid} | awk '{print $2}'`.to_i / 1024

    open(file_path, 'a') { |f|
      f.puts [Time.now.to_i, rss_in_mbyte, GC.stat[:heap_live_slots]].join(', ')
    }
